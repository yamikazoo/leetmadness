...
// Function to check the submission result and trigger audio changes
function checkForIncorrectSubmission() {
  ...
  if (errorElement && !messageShown) {
    ...
    if (
      ...
      chrome.runtime.sendMessage({ command: 'makeAudioWorse' }); // Trigger audio distortion
    ...
    if (successText.includes("Accepted")) {
      chrome.runtime.sendMessage({ command: 'makeAudioBetter' }); // Trigger audio improvement
      messageShown = true; // Set the flag to true
    }
  } else {
